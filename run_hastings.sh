...
hydra-genetics create-input-files -f -p Illumina -d ${inbox_path}/ -t N  -b 'NNNNNNNNN+NNNNNNNNN' --data-json samples_and_settings.json --data-columns ${pipeline_path}/config/wp3_te_columns.json 
...
if [ $(wc -l < samples_comprehensive.tsv) -gt 1 ]; then
    ...
    mv units_comprehensive.tsv config/units.tsv
    ...
    mv results/multiqc_report.html results/multiqc_report_comprehensive.html
    mv results/peddy.html results/peddy_comprehensive.html
...
if [ $(wc -l < samples_twist2.0.tsv) -gt 1 ]; then
    ...
    mv samples_with_info.tsv config/samples.tsv
    mv units_twist2.0.tsv config/units.tsv
